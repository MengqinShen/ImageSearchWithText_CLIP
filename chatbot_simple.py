...
class SimpleRAGChatbot:
    ...
    def display_images(self, best_photo_ids, image_ids, query):
        ...
        try:
            ...
            temp_file = f"temp_search_results_{hash(query) % 10000}.png"
    ...
    def chat(self, message, history):
        ...
        try:
            ...
            if best_photo_ids and image_ids:
                # Create image grid
                image_grid = self.display_images(best_photo_ids, image_ids, message)
                
                if image_grid:
                    response = f"{status_msg} Here are the images:"
                    history.append({"role": "assistant", "content": response})
                    return "", history, image_grid
                else:
...
def create_chatbot_interface():
    ...
    with gr.Blocks(title="RAG Image Search Chatbot", theme=gr.themes.Soft()) as demo:
        ...
        gr.Markdown("Chat with me to search for images!")
